...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `SmartVenetianBlinds`
...
- Inherit from both platform entity and `SmartVenetianBlindsEntity` (order matters)
...
from custom_components.smart_venetian_blinds.sensor import async_setup_entry
...
**Config entry data:** `entry_data: SmartVenetianBlindsData = hass.data[DOMAIN][entry.entry_id]`
