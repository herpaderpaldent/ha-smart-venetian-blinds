"""Tests for SmartVenetianBlindsDataUpdateCoordinator._calculate_slat_angle."""
...
class TestCalculateSlatAngle:
    """Tests for _calculate_slat_angle return values."""

    def test_returns_none_when_sun_below_horizon(self) -> None:
        """Returns None when sun is below the horizon."""
        coordinator = _make_coordinator(sun_position=_SUN_BELOW_HORIZON)
        result = coordinator._calculate_slat_angle()
        assert result is None

    def test_returns_result_when_sun_on_facade(self) -> None:
        """Returns a SlatCalculationResult when sun is actively hitting the facade."""
        coordinator = _make_coordinator(sun_position=_SUN_ON_SOUTH_FACADE)
        result = coordinator._calculate_slat_angle()
        assert result is not None
        assert result.sun_is_behind_facade is False

    def test_returns_result_when_sun_behind_facade(self) -> None:
        """Returns a SlatCalculationResult with sun_is_behind_facade=True when sun passed."""
        coordinator = _make_coordinator(sun_position=_SUN_BEHIND_SOUTH_FACADE)
        result = coordinator._calculate_slat_angle()
        assert result is not None
        assert result.sun_is_behind_facade is True

    def test_returns_none_when_no_sun_position(self) -> None:
        """Returns None when sun provider has no position."""
        coordinator = _make_coordinator(sun_position=None)
        result = coordinator._calculate_slat_angle()
        assert result is None
