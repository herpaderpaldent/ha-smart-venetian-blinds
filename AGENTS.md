...
- **Repository:** herpaderpaldent/ha-smart-venetian-blinds

**Key directories:**

- `custom_components/smart_venetian_blinds/` - Main integration code
...
- Integration logs: `custom_components.smart_venetian_blinds: debug` in `config/configuration.yaml`
...
- Use the domain `smart_venetian_blinds` for all DOMAIN references
- Prefix all integration-specific classes with `SmartVenetianBlinds`
- Use "Smart Venetian Blinds" as the display title
...
- Inherit from platform base + `SmartVenetianBlindsEntity`
...
- `tests/` mirrors `custom_components/smart_venetian_blinds/` structure
