...
This document describes all configuration options and settings available in the Smart Venetian Blinds custom integration.
...
### `smart_venetian_blinds.example_service`
...
```yaml
service: smart_venetian_blinds.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: smart_venetian_blinds.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Smart Venetian Blinds Alert
...
- [GitHub Issues](https://github.com/herpaderpaldent/ha-smart-venetian-blinds/issues) - Report problems
